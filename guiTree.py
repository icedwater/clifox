...
#  self.screen.move(self.screen.getyx()[0]+1,0)
 ...
 def markCollapsed(self,root):
  ...
  while i<len(self.list):
   ...
   if self.levels.get(self.list[i],level)<level:
    break
   if self.states[self.list[i]]==1: self.collapseFunc(self.list[i])
 ...
 def collapse(self,i=None):
  ...
  self.l("collapse:","cur:",i,"canCollapse:",self.canCollapse(i),"state:",self.states[i])
  if self.canCollapse(i) and self.states.get(i,1)==1:
   newIndex=self.markCollapsed(i)
   startIndex=self.list.index(i)+1
   rem=[self.list.pop(startIndex) for j in xrange(startIndex,newIndex)]
   rem.insert(0,i)
...
#  self.screen.addstr(0,0,str(self.list)[:self.maxx])
 ...
 def collapseEntireList(self):
  ...
  while sum(self.states.values())>0:
   self.collapse([i for i in self.list[::-1] if self.states[i]==1][0])
   t+=1
  return t
...
#  title,uri=i.title,i.uri
   if i.hasChildren:
    ...
    [ret.append((j.title if j.title!=None else '',j)) for j in children]
  return ret
